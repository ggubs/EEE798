...
B = abs(reshape(B1,length(phi1),length(theta1)));
B_normed = abs(B)/(abs(max(B(:))));
B_shifted = B_normed - min(B_normed(:));

ax = axes();
daspect([1 1 1])
hold on
add_x_y_z_labels(ax,max(B_shifted(:)));
add_az_el_labels(ax,max(B_shifted(:)));
draw_circle(ax, 90, 1:1:360,'--mw-graphics-colorSpace-rgb-blue','XY',1.1*max(B_shifted(:))); % Circle in the x-y plane
draw_circle(ax, 1:1:360, 0,'--mw-graphics-colorSpace-rgb-green','XZ',1.1*max(B_shifted(:))); % Circle in the x-z plane
draw_circle(ax, 1:1:360,90, '--mw-graphics-colorSpace-rgb-red','YZ',1.1*max(B_shifted(:))); % Circle in the y-z plane
...
% Plot surface
surf(ax,BX,BY,BZ,B_shifted, 'FaceColor','interp','LineStyle','none','FaceAlpha',1.0,'Tag','3D polar plot');
xlabel('dB Normalized')
...
set(gcf,'Position',[100 100 600 600])
    ...
    function add_az_el_labels(axes1, rad)
        ...
        arrow3(az_arrow_start, az_arrow_end, 'k-3', 2, 2);
        text(axes1,rad*1.02,rad*0.12,0.0, texlabel('az'));

        arrow3(el_arrow_start, el_arrow_end,'k-3', 2, 2);
